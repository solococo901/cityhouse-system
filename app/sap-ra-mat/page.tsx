...
export default function ActiveProjectsSection() {
  ...
  const breadcrumbData = [
    ...
    { label: "Sắp ra mắt" } // Mục hiện tại không cần href
  ];
      ...
      <main className="min-h-screen font-['Inter',_sans-serif] bg-[#FDFDFD]">
        ...
        <section id="portfolio" className="w-full">
          ...
          <div className="max-w-[1280px] mx-auto px-4 md:px-6 py-12 md:py-20">
            ...
            <div className="border rounded-xl border-slate-200 bg-white p-0 rounded-none shadow-sm mb-10">
              ...
              <div className="flex gap-1 overflow-x-auto p-3 border-b border-slate-100 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:flex-wrap bg-slate-50/30">
                {brandTabs.map((brand) => (
                  <button
                    ...
                    className={`whitespace-nowrap px-4 rounded-xl py-1.5 text-[11px] font-bold uppercase tracking-widest rounded-none border transition-all duration-300 ${
                      brand === activeBrand
            ...
            {filteredProjects.length > 0 ? (
              <div className="grid grid-cols-1 rounded-xl overflow-hidden md:grid-cols-2 xl:grid-cols-3 gap-0 border border-slate-200 bg-white divide-y md:divide-y-0 md:border-b-0">
                {filteredProjects.map((project) => (
                  <Link href={`/sap-ra-mat/${project.id}`} key={project.id} className="block w-full">
                    <article
                    ...
                    >
                      <div>
                        <div className="relative aspect-[3/2] overflow-hidden rounded-none bg-slate-100 mb-6">
                          ...
                          />
                          <div className="absolute rounded-xl inset-0 bg-gradient-to-t from-[#0F1A41]/30 via-transparent to-transparent opacity-60" />
